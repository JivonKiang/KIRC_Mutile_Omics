...
for (dir in sub_dirs) {
  ...
  has_valid_file <- any(sapply(dir_files, function(file) {
    "Single SNP" %in% excel_sheets(file)
  }))
  
  if (has_valid_file) {
    # 筛选该文件夹中包含目标工作表的文件
    valid_dir_files <- sapply(dir_files, function(file) {
      if ("Single SNP" %in% excel_sheets(file)) file else NA
    })
...
data_list <- lapply(valid_files, function(file) {
  read_excel(file, sheet = "Single SNP")###      Leave-One-Out
})
...
gene_processed <- na.omit(combined_gene[,-5])

gene_processed$exposure <- gene_processed$id.exposure

colnames(gene_processed)[8] <- c("pval")
...
# ==================== 执行保存操作 ====================
# 保存 cell 数据
save_multiple_sheets(cell_corrected, "cell_results single snp.xlsx")

# 保存 gene 数据
save_multiple_sheets(gene_corrected, "gene_results single snp.xlsx")
...
# 基因数据处理 ---------------------------------------------------------------
# 生成图表
pdf("upset/gene_upset  single snp.pdf", width=8, height=6)
...
png("upset/gene_upset  single snp.png", width=2400, height=1800, res=300) # 高分辨率PNG[3](@ref)
upset(fromList(gene_list),
      nsets = 99, nintersects = 20,
      mainbar.y.label = "Significant Combinations",
      sets.x.label = "p-value Types Count")
...
write.xlsx(gene_intersect_matrix, "upset/gene_intersections  single snp.xlsx", 
...
# 生成图表（重复基因流程）
pdf("upset/cell_upset  single snp.pdf", width=8, height=6)
...
png("upset/cell_upset  single snp.png", width=2400, height=1800, res=300)
upset(fromList(cell_list),
      nsets = 99, nintersects = 20,
      mainbar.y.label = "Significant Combinations",
      sets.x.label = "p-value Types Count")
...
write.xlsx(cell_intersect_matrix, "upset/cell_intersections  single snp.xlsx",
